...
class RNNPhonemeClassifier(object):
    ...
    def __init__(self, input_size, hidden_size, output_size, num_layers=2):
        ...
        self.rnn = [
             # for the first layer, x1, h01
             RNNCell(input_size, hidden_size)
             if i == 0
             # for the second layer , h11, h02 etc
    ...
    def forward(self, x, h_0=None):
        ...
        for s in range(seq_len):
            ...
            for l in range(self.num_layers):
                #we need to use the last hiddens to compute the new one.
                h_il = self.rnn[l].forward(x, self.hiddens[-1][l])
                hiddens.append(h_il)
                #first layer hidden will be the second layers x.
                x = h_il
            self.hiddens.append(hiddens.copy())

        #the last hil = h32, past to the linear layer.
        ...
        for t in range(seq_len-1,-1,-1):
            for l in range(self.num_layers-1, -1,-1):
                ...
                if l != 0:
                    ...
                    h_prev_l = self.x[:,t,:] #xt
                h_prev_t = self.hiddens[t][l]
                dxl, dhl = self.rnn[l].backward(dh[l], h, h_prev_l, h_prev_t)
                #update dh
                dh[l] = dhl
                if l !=0:
                    dh[l-1]+= dxl
