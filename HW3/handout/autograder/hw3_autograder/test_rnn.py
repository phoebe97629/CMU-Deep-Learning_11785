...
class RNNTest(Test):
    ...
    def test_rnn_classifier(self):
        ...
        assert np.allclose(my_out, ref_out, rtol=1e-03), 'wrong value in rnn classifier forward'
        ...
        assert np.allclose(my_dh, dh.detach().numpy(), rtol=1e-04), 'wrong value for dh in rnn classifier backward'
        assert np.allclose(
            my_rnn_model.output_layer.dW,
            grad_dict["output.weight"].detach().numpy(),
            rtol=1e-03,
        ), 'wrong value for dW in rnn classifier backward'
        assert np.allclose(
            my_rnn_model.output_layer.db, grad_dict["output.bias"].detach().numpy()
        ), 'wrong value for db in rnn classifier backward'
    ...
    def gen_test(self):
        file = os.path.join(
                    "autograder", "hw3_autograder", "data", "toy_rnncell_backward.npy"
                )
