...
class CNN(object):
    ...
    def __init__(self, input_width, num_input_channels, num_channels, kernel_sizes, strides,
        ...
        for i in range(self.nlayers):
            self.convolutional_layers.append(Conv1D(in_c, num_channels[i],
                                                    kernel_sizes[i],
                                                    strides[i],
                                                    conv_weight_init_fn,
                                                    bias_init_fn))
        ...
        self.linear_layer = Linear(in_c*output_size,
                                   num_linear_neurons,
    ...
    def forward(self, x):
        ...
        for l in range(self.nlayers):
            z = self.convolutional_layers[l].forward(self.input)
            self.input = self.activations[l].forward(z)
        self.input = self.flatten.forward(self.input)
        self.output = self.linear_layer.forward(self.input)
        return self.output
    ...
    def backward(self, labels):
        ...
        for i in range(self.nlayers-1, -1,-1):
            dz_cur = grad * self.activations[i].derivative()  
            grad = self.convolutional_layers[i].backward(dz_cur)          
        dz = dz_cur        
        return grad        
    ...
    def zero_grads(self):
        ...
        for l in range(self.nlayers):
            self.convolutional_layers[l].dW.fill(0.0)
            self.convolutional_layers[l].db.fill(0.0)
    ...
    def step(self):
        # Do not modify this method
        for l in range(self.nlayers):
            self.convolutional_layers[l].W = (self.convolutional_layers[l].W -
                                              self.lr * self.convolutional_layers[l].dW)
            self.convolutional_layers[l].b = (self.convolutional_layers[l].b -
                                              self.lr * self.convolutional_layers[l].db)
