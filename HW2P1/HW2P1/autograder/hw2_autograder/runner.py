...
def test_cnn_correctness():
    ...
    print('Section 4.1.1 - Forward | 10 points')
    print('Conv1D Forward:', 'PASS' if a == 1 else 'FAIL')

    print('Section 4.1.2 - Backward | 20 points')
...
def conv2d_forward_correctness():
    ...
    test_model = Conv2D(in_c, out_c, kernel, stride, random_normal_weight_init_fn, np.ones)
...
def conv2d_dilation_forward_correctness(idx):

    scores_dict_1 = [0,0,0,0]
    ...
    kernel, stride =  rint(1,10), rint(1,10)
    batch, width = rint(1,4), rint(20,300)
    padding , dilation = rint(1,10), rint(1,10)
    ...
    if idx == 0:
        print("---Conv2D_dilation Forward (Tests)---")
    try:
        net = Conv2D_dilation(in_c, out_c, kernel, stride, padding, dilation, random_normal_weight_init_fn, np.ones)
    except:
        info()
        print('Failed to pass parameters to your Conv2D_dilation function!')
        return scores_dict_1

    model = nn.Conv2D_dilation(in_c, out_c, kernel, stride)
    model.weight = nn.Parameter(torch.tensor(net.W))
    model.bias = nn.Parameter(torch.tensor(net.b))
    ...
    if idx == 0:
        print("---Conv2D_dilation Backward (Tests)---")
    assert dx.shape == x.shape
    assert net.dW.shape == model.weight.grad.detach().numpy().shape
    assert net.db.shape == model.bias.grad.detach().numpy().shape
    ...
    if min(scores_dict_1) != 1:
        info()
        if scores_dict_1[1] == 0:
            print('Fail to return correct backward values dx')
        if scores_dict_1[2] == 0:
            print('Fail to return correct backward values dW')
        if scores_dict_1[3] == 0:
            print('Fail to return correct backward values db')
    return scores_dict_1

def conv2d_dilation_forward_correctness():
    scores = []
    worker = min(mtp.cpu_count(),4)
    p = mtp.Pool(worker)

    for __ in range(15):
        scores_dict_1 = test_cnn_correctness_once(__)
        scores.append(scores_dict_1)
        if min(scores_dict_1) != 1:
            return False

    # scores = np.array(scores).min(0)
    a, b, c, d = np.array(scores).min(0)
    print('Section 4.3.1 - Forward | 5 points')
    print('Conv2D_dilation Forward:', 'PASS' if a == 1 else 'FAIL')

    print('Section 4.3.2 - Backward | 5 points')
    print('Conv2D_dilation dX:', 'PASS' if b == 1 else 'FAIL')
    print('Conv2D_dilation dW:', 'PASS' if c == 1 else 'FAIL')
    print('Conv2D_dilation db:', 'PASS' if d == 1 else 'FAIL')
...
tests = [
    ...
    {
        'name': '4.3 - Padding and Dilation | 10 points',
        'autolab': 'Padding-Dilation',
        'handler': conv2d_dilation_forward_correctness,
        'value': 10,
    },
    {
