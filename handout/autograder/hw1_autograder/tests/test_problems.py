...
def test_adamW():
    ...
    # load the actual solutions
    solwW = pickle.load(open(base_dir + "adamW_solW.pkl", "rb"))
    solwb = pickle.load(open(base_dir + "adamW_solb.pkl", "rb"))
    ...
    num_test_updates = 5
    optim = optimizer.adamW(mlp, 0.9, 0.999, 0.01)
    for u in range(num_test_updates):
        mlp.zero_grads()
        mlp.forward(x)
        mlp.backward(y)
    ...
    for i, (pred, gt) in enumerate(zip(W, solwW)):
        closeness_test(pred, gt, "mlp.linear_layers[%d].W" % i)

    for i, (pred, gt) in enumerate(zip(b, solwb)):
        closeness_test(pred, gt, "mlp.linear_layers[%d].b" % i)
